      ...
      <Section className="bg-brand-ice">
        <Container className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {industries.map((industry) => <div key={industry.slug} className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-border"><h3 className="text-xl font-black">{industry.title.replace(" Services", "")}</h3><p className="mt-3 text-sm leading-6 text-muted-foreground">{industry.description}</p></div>)}
          </div>
        </Container>
      </Section>
      <Section>
