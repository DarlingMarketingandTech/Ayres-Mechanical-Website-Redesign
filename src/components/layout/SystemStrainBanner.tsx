...
function SystemStrainBannerInner() {
  ...
  useEffect(() => {
    ...
    async function resolveWeather() {
      try {
        const fallbackState = await loadWeatherForCoordinates(
          INDIANAPOLIS_COORDINATES,
          abortController.signal,
          "default",
        );

        if (!abortController.signal.aborted) {
          setSystemStrain(fallbackState);
        }
      } catch {
        if (!abortController.signal.aborted) {
          setSystemStrain(null);
        }
        return;
      }
